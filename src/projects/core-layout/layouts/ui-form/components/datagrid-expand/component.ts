...
export class UiDatagridExpandComponent implements OnInit, OnDestroy {
    ...
    animals = ['Cat', 'Dog', 'Horse', 'Mouse', 'Fish'];
    ...
    /* Ui Lists. */
    public genderOptions: Array<SelectedItem> = [
        {id: 'Male', text: 'Male'},
        {id: 'Female', text: 'Female'},
    ];
    public currencyOptions: Array<SelectedItem> = [
        {id: '$', text: 'Dollars ($)'},
        {id: '€', text: 'Euros (€)'},
        {id: '£', text: 'Pounds (£)'},
    ];
    public availableOptions: Array<SelectedItem> = [
    ...
    constructor(private _fb: FormBuilder,
        ...
        this.subscriptionsArray.push(this.searchForm.valueChanges.subscribe((form) => this.requestSearch(form)));

        for (let i = 0; i < 1000; i++) {
            const startDateRef = this.convertDate(new Date(+(new Date()) - Math.floor(Math.random() * 10000000000)));
            this.fakeDatas.push({
                id: Math.floor(Math.random() * Math.floor(1000)),
                name: this.names[Math.floor(Math.random() * Math.floor(this.names.length))],
                age: 22 + Math.floor(Math.random() * Math.floor(50)),
                startDate: startDateRef,
                endDate: this.convertDate(new Date(startDateRef).setDate(new Date(startDateRef).getDate() + Math.floor(Math.random() * 1000))),
                animal: this.animals[Math.floor(Math.random() * Math.floor(this.animals.length))],
                sport: this.sports[Math.floor(Math.random() * Math.floor(this.sports.length))],
                country: this.countries[Math.floor(Math.random() * Math.floor(this.countries.length))].text,
                color: this.colors[Math.floor(Math.random() * Math.floor(this.colors.length))],
                gender: this.genders[Math.floor(Math.random() * Math.floor(this.genders.length))],
                amount: Math.floor(Math.random() * Math.floor(1000000)),
                currency: this.currencies[Math.floor(Math.random() * Math.floor(this.currencies.length))],
                codeBank: 1000 + Math.floor(Math.random() * Math.floor(1000)),
                password: Math.random().toString(36).substring(7),
                available: this.available[Math.floor(Math.random() * Math.floor(this.available.length))],
            });
        }
        this.requestSearch(this.searchForm);
    }
    ...
    requestSearch(form) {
        this.filteredDatas = [];
        let idList = [];
        for (let i = 0; i < this.fakeDatas.length; i++) {
            this.filteredDatas.push(this.fakeDatas[i]);
        }
        console.clear();
        // AGE
        if (this.searchForm.get('age').value !== '') {
            ...
            if (idList.length > 0) {
                for (let i = 0; i < idList.length; i++) {
                    this.filteredDatas.splice(idList[i] - i,1);
                }
            }
        }
        // Genders
